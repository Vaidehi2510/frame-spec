...
June 2026  |  Confidential  |  Revision 5
...
Crucially, Nebari is open-source and additive.  It is also a representative example (and OpenTeams’ contribution to a broader collection) of open source tools that forms the foundation of AI infrastructure. Most organizations will already be using some parts of the open source ecosystem and so in that way we can say most organizations are already “using some part of Nebari”  The organization might obtain these components without ever having known the Nebari collection, but when they learn about it, it makes it easier to adopt additional parts of Nebari to their internal infrastructure.  This allows the openness to become not a liability, but a strategic moat. The same dynamic that made NumPy the universal array standard (and made the rest of the scientific Python ecosystem possible) applies here: when the infrastructure layer is open, trust compounds and adoption accelerates. Travis Oliphant, the original creator of NumPy and a founder of Anaconda, brings exactly this institutional credibility and ecosystem-building experience to the Nebari project. Dharhas Pothina, who has been guiding Nebari's development for the past six years, brings deep experience deploying open source in the enterprise and in government to open-source projects.
...
* It defines the common format by which Frames, Cogs, and Ops can be packaged for distribution and redeployed in billions of installations.
...
## **3.3  The Accountable Intelligence Hub — the center of Organizational Deployment**

An Intelligence Hub is a particular configuration of open source components that are deployed inside an organizational perimeter. Based on Nebari and packaged with Nebi, it is the concrete realization of the standard: a running system that integrates models, data, business systems, workflows, and the organization's accumulated Cogs, Frames, and Ops into a unified, governed AI control plane.
...
Organizational Memory is the persistent context substrate of the Intelligence Hub. It is what turns the Hub from a server-side deployment of AI tools into a compound learning system that accumulates and applies the organization's accumulated context, interactions, and outcomes over time.

This concept is implied throughout the architecture of the distributed AI economy, but warrants explicit treatment. Frames give the organization a way to encode *intentional* context — what they have chosen to make explicit and shareable with defined granularity.  Cogs and Ops generate *emergent* context encapsulating what actually happens when AI work is performed. Organizational Memory is where both kinds of context live, accumulate, and become available to future work.
...
The Intelligence Hub does not prescribe a particular technology for Organizational Memory — it provides the integration points and governance hooks. Organizations can compose their Organizational Memory from any combination of:
...
* The tools and APIs it has access to and the harness around it.
...
| Nebari is the open-source ecosystem that integrates with the underlying OS, |
...
OpenTeams  |  nebari.dev  |  Confidential — June 2026  |  Revision 5
